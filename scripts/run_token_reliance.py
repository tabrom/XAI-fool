...
def main():
    ...
    parser.add_argument("--expl_method", type=str, default='LRP', help="Explanation method to use when getting attributions: gradients, integrated_gradients, etc.")
    ...
    def tokenize_and_attach(batch): # tokenizer and args defined outside 
        ...
        for i, (sample, batch_row) in enumerate(zip(enc['input_ids'], rows)):
            ...
            if args.approach == 'location':
                ...
                if len(og_expl) == 0: # empty explanation - only for test set 
                    ...
                    continue
                top_idxs = np.argpartition(og_expl,-args.k)[-args.k:] # get indices of top-k largest values, not sorted
                expl_mask[top_idxs] = 1
            elif args.approach == 'tokens' or args.approach == 'tokens_unk':
    ...
    if args.model_name == "custom-bert":
        if args.run_id is not None:
            print("Loading finetuned custom BERT model from wandb run ID:", args.run_id)
        ...
        if args.run_id is not None:
            print("Loading finetuned XAI-Albert model from wandb run ID:", args.run_id)
            model = load_xai_albert(model_name=args.model_name, device=device, run_id=args.run_id)
        else:
            print("Loading XAI-Albert model without finetuning.")
            model = load_xai_albert(model_name=args.model_name, device=device)
    else:
    ...
    if args.approach == "tokens_unk":
        tokenized = tokenized.filter(
            lambda expl_mask: (np.array(expl_mask) != 0).any(),  # or use has_target_token
            input_columns=["expl_mask"],
        )
    ...
    if args.get_attributions or args.eval_only: # gets attributions for train and val, bs=1, matches og method exactly 
        ...
        if args.get_attributions:
            ...
            if not args.eval_only:
                save_file = get_explanations_path(args, dataset_split='val', use_results=True).replace('.json', f'{run.id}.json')
            else:
                r_id = np.random.randint(0, 1_000_000)
                save_file = get_explanations_path(args, dataset_split='val', use_results=True).replace('.json', f'_eval_only_{r_id}.json')
