...
def compute_GAE_attr(model):
    # torch rewritten version of the one implemented in OG Ali paper 
    # + fixed summing over columns vs extracting CLS token 
    # ++ not doing CLS-to-CLS zeroing anymore
    ...
    attentions_mat = attentions_mat.mean(dim=2) # should be (L, B, T, T)
    ...
    # joint_attentions[:, 0, 0] = 0 # not zeroing out, need to find other solution for CLS-to-CLS issue
    
    attribution = joint_attentions[:,0,:]  # take CLS row should be (B, T) 
    return attribution
